...
- `redis`: use self-hosted Redis as storage method. Supports multi-tenancy and can be used for development and production. Requires `REDIS_URL` (or `REDIS_HOST` and `REDIS_PORT`) environment variables to be set. See [REDIS_SETUP.md](./REDIS_SETUP.md) for detailed configuration instructions.
